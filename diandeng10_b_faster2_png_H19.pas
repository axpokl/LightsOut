...
if (not matInit) or (longint(n)<lastLN) or (longint(n)<lastFN) then
  begin
  VecZero(y2); VecZero(y1); VecZero(y02); VecZero(y01);
  VecZero(f2); VecZero(f1); VecZero(c2); VecZero(c1);
  SetBit(f1,0,1); SetBit(c1,0,1);
  lastLN:=0; lastFN:=0; matInit:=true;
  end;
for j:=lastLN+1 to n do
  begin
  VecZero(y03); VecZero(y3);
  VecShiftL1(v1,y01);
  VecShiftR1(v2,y01);
  VecCopy(y03,v1);
  VecXorEq(y03,y01);
...
lastFN:=n;
VecCopy(y,y1); VecCopy(y0,y01); VecCopy(f,f1); VecCopy(c,c1);
end;
