...
{ H79: the same 64+96 split, six-product formulas and reused diagonal as A. }
...
ax:=a[ao+0] xor a[ao+2]; bx:=b[bo+0] xor b[bo+2];
g0:=0; if (ax<>0) and (bx<>0) then g0:=CLMul32(ax,bx);
ax:=a[ao+1] xor a[ao+3]; bx:=b[bo+1] xor b[bo+3];
g1:=0; if (ax<>0) and (bx<>0) then g1:=CLMul32(ax,bx);
ax:=a[ao+0] xor a[ao+1] xor a[ao+2] xor a[ao+3]; bx:=b[bo+0] xor b[bo+1] xor b[bo+2] xor b[bo+3];
g01:=0; if (ax<>0) and (bx<>0) then g01:=CLMul32(ax,bx);
ax:=a[ao+0] xor a[ao+2] xor a[ao+4]; bx:=b[bo+0] xor b[bo+2] xor b[bo+4];
g02:=0; if (ax<>0) and (bx<>0) then g02:=CLMul32(ax,bx);
ax:=a[ao+1] xor a[ao+3] xor a[ao+4]; bx:=b[bo+1] xor b[bo+3] xor b[bo+4];
g12:=0; if (ax<>0) and (bx<>0) then g12:=CLMul32(ax,bx);
e1:=e1 xor e0 xor e2;
f01:=f01 xor f0 xor f1;
f02:=f02 xor f0 xor f1 xor f2;
f12:=f12 xor f1 xor f2;
g01:=g01 xor g0 xor g1;
g02:=g02 xor g0 xor g1 xor f2;
g12:=g12 xor g1 xor f2;
g0:=e2 xor e0 xor f0 xor g0;
g01:=e1 xor f01 xor g01;
g02:=e2 xor f02 xor g02 xor f0;
g12:=f12 xor g12 xor f01;
