...
var lo,e0,e1,sh,tmp:longint; c00,c01,c10,c11:LongWord;
...
var i,lim:longint; ex,ey:array[0..8] of longint;
var cx,cy,l,h,m,z0,z2,z1,mid:LongWord;
begin
lim:=dx; if dy>lim then lim:=dy; lim:=(lim+32) shr 5;
cx:=0; cy:=0;
for i:=0 to lim-1 do
  begin
  ex[0]:=(x[i] shr 0) and $FF;
  ex[1]:=(x[i] shr 8) and $FF;
  ex[3]:=(x[i] shr 16) and $FF;
  ex[4]:=(x[i] shr 24) and $FF;
  ey[0]:=(y[i] shr 0) and $FF;
  ey[1]:=(y[i] shr 8) and $FF;
  ey[3]:=(y[i] shr 16) and $FF;
  ey[4]:=(y[i] shr 24) and $FF;
  ex[2]:=ex[0] xor ex[1];
  ex[5]:=ex[3] xor ex[4];
  ex[6]:=ex[0] xor ex[3];
  ex[7]:=ex[1] xor ex[4];
  ...
  ey[7]:=ey[1] xor ey[4];
  ey[8]:=ey[2] xor ey[5];
  l:=mul8[ix[0] or ex[0]] xor mul8[ix[1] or ey[0]];
  h:=mul8[ix[2] or ex[1]] xor mul8[ix[3] or ey[1]];
  m:=mul8[ix[4] or ex[2]] xor mul8[ix[5] or ey[2]];
  z0:=l xor ((m xor l xor h) shl 8) xor (h shl 16);
  l:=mul8[ix[6] or ex[3]] xor mul8[ix[7] or ey[3]];
  h:=mul8[ix[8] or ex[4]] xor mul8[ix[9] or ey[4]];
  m:=mul8[ix[10] or ex[5]] xor mul8[ix[11] or ey[5]];
  z2:=l xor ((m xor l xor h) shl 8) xor (h shl 16);
  l:=mul8[ix[12] or ex[6]] xor mul8[ix[13] or ey[6]];
  h:=mul8[ix[14] or ex[7]] xor mul8[ix[15] or ey[7]];
  m:=mul8[ix[16] or ex[8]] xor mul8[ix[17] or ey[8]];
  z1:=l xor ((m xor l xor h) shl 8) xor (h shl 16);
  mid:=z0 xor z1 xor z2; x[i]:=z0 xor (mid shl 16) xor cx;
  cx:=z2 xor (mid shr 16);
  l:=mul8[ix[18] or ex[0]] xor mul8[ix[19] or ey[0]];
  h:=mul8[ix[20] or ex[1]] xor mul8[ix[21] or ey[1]];
  m:=mul8[ix[22] or ex[2]] xor mul8[ix[23] or ey[2]];
  z0:=l xor ((m xor l xor h) shl 8) xor (h shl 16);
  l:=mul8[ix[24] or ex[3]] xor mul8[ix[25] or ey[3]];
  h:=mul8[ix[26] or ex[4]] xor mul8[ix[27] or ey[4]];
  m:=mul8[ix[28] or ex[5]] xor mul8[ix[29] or ey[5]];
  z2:=l xor ((m xor l xor h) shl 8) xor (h shl 16);
  l:=mul8[ix[30] or ex[6]] xor mul8[ix[31] or ey[6]];
  h:=mul8[ix[32] or ex[7]] xor mul8[ix[33] or ey[7]];
  m:=mul8[ix[34] or ex[8]] xor mul8[ix[35] or ey[8]];
  z1:=l xor ((m xor l xor h) shl 8) xor (h shl 16);
  mid:=z0 xor z1 xor z2; y[i]:=z0 xor (mid shl 16) xor cy;
  cy:=z2 xor (mid shr 16);
  end;
x[lim]:=cx; y[lim]:=cy;
...
SetLength(r,count shr 4); SetLength(work,count div 16+64);
...
BuildUComboHalfKernelW(va,vb,degmax,1,combo);
...
if mode=2 then BuildFibonacciHalfKernelW(hi+1,ha,convWords)
else
  begin
