...
procedure CalcGCDRowOnly(var gout:TVec);
var qtmp:TVec;
begin
TimeMark('q');
gcd(f,c,gout,qtmp);
...
var fh:TBmpFileHeader;
var ih:TBmpInfoHeader;
var pal0,pal1:TRGBQuad;
begin
  bw.width:=width;
  bw.height:=height;
  bw.rowRaw:=(width+7) div 8;
  bw.rowPad:=(bw.rowRaw+3) and not 3;
  SetLength(bw.rowBuf,bw.rowPad);

  Assign(bw.f,fn);
  Rewrite(bw.f,1);

  fh.bfType:=$4D42;
  fh.bfOffBits:=SizeOf(fh)+SizeOf(ih)+2*SizeOf(TRGBQuad);
  fh.bfReserved1:=0;
  fh.bfReserved2:=0;
  fh.bfSize:=fh.bfOffBits + LongWord(bw.rowPad)*LongWord(height);
  ...
  ih.biBitCount:=1;
  ih.biCompression:=0;
  ih.biSizeImage:=LongWord(bw.rowPad)*LongWord(height);
  ih.biXPelsPerMeter:=3780;
  ih.biYPelsPerMeter:=3780;
  ih.biClrUsed:=2;
  ih.biClrImportant:=2;

  pal0.b:=255; pal0.g:=255; pal0.r:=255; pal0.a:=0;
  pal1.b:=0; pal1.g:=0; pal1.r:=0; pal1.a:=0;

  BlockWrite(bw.f,fh,SizeOf(fh));
  BlockWrite(bw.f,ih,SizeOf(ih));
  BlockWrite(bw.f,pal0,SizeOf(pal0));
  BlockWrite(bw.f,pal1,SizeOf(pal1));
end;

procedure BMP1WriteVecRow(var bw:TBmp1Writer; const row:TVec; rowLen:LongInt);
var xi,lim:LongInt;
begin
  FillChar(bw.rowBuf[0],bw.rowPad,0);
  lim:=rowLen;
  if lim>bw.width then lim:=bw.width;
  for xi:=0 to lim-1 do
    if ((row[xi shr 5] shr (xi and 31)) and 1)<>0 then
      bw.rowBuf[xi shr 3]:=bw.rowBuf[xi shr 3] or (Byte(1) shl (7-(xi and 7)));
...
BMP1Close(bw);
WriteFullBMPAndCheck:=GeneMatCheckOnly();
end;

procedure WriteGCDBMP(const fn:ansistring);
var bw:TBmp1Writer;
var grow:TVec;
begin
BMP1Open(bw,fn,gcdN,gcdN);
for n:=0 to fullN do
  begin
  PrepN;
  MakeMat();
  CalcGCDRowOnly(grow);
  BMP1WriteVecRow(bw,grow,n+1);
  end;
BMP1Close(bw);
end;
...
MakeMat();
CalcMat2();
ok:=WriteFullBMPAndCheck('png_T10000/10000.bmp');
writeln(ok);

BMP1Open(bw,'png_T10000/1-10000.bmp',summaryN,summaryN);
...
WriteGCDBMP('png_T10000/1-10000_gcd.bmp');
end.
