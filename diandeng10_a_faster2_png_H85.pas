...
var a0,a1,at:TS; m00,m01,m10,m11,mt:TM;
var lo,e0,e1,sh,j,k,tmp:longint; c00,c01,c10,c11:LongWord;
var ix:array[0..35] of LongWord;
procedure ApplyPair32(var x,y:TL; var dx,dy:longint);
var i,j,lim:longint; ex,ey:array[0..8] of longint;
var cx,cy:TM; p:array[0..35] of ^TMul16Bool;
var z:array[0..11] of boolean; l0,l1,h0,h1,m0,m1:boolean;
begin
lim:=dx; if dy>lim then lim:=dy; lim:=(lim+32) shr 5;
cx:=Default(TM); cy:=Default(TM);
for i:=0 to lim-1 do
  begin
  ex[0]:=(ord(x[i*32+0]) shl 0) or (ord(x[i*32+1]) shl 1) or (ord(x[i*32+2]) shl 2) or (ord(x[i*32+3]) shl 3) or (ord(x[i*32+4]) shl 4) or (ord(x[i*32+5]) shl 5) or (ord(x[i*32+6]) shl 6) or (ord(x[i*32+7]) shl 7);
  ex[1]:=(ord(x[i*32+8]) shl 0) or (ord(x[i*32+9]) shl 1) or (ord(x[i*32+10]) shl 2) or (ord(x[i*32+11]) shl 3) or (ord(x[i*32+12]) shl 4) or (ord(x[i*32+13]) shl 5) or (ord(x[i*32+14]) shl 6) or (ord(x[i*32+15]) shl 7);
  ex[3]:=(ord(x[i*32+16]) shl 0) or (ord(x[i*32+17]) shl 1) or (ord(x[i*32+18]) shl 2) or (ord(x[i*32+19]) shl 3) or (ord(x[i*32+20]) shl 4) or (ord(x[i*32+21]) shl 5) or (ord(x[i*32+22]) shl 6) or (ord(x[i*32+23]) shl 7);
  ex[4]:=(ord(x[i*32+24]) shl 0) or (ord(x[i*32+25]) shl 1) or (ord(x[i*32+26]) shl 2) or (ord(x[i*32+27]) shl 3) or (ord(x[i*32+28]) shl 4) or (ord(x[i*32+29]) shl 5) or (ord(x[i*32+30]) shl 6) or (ord(x[i*32+31]) shl 7);
  ey[0]:=(ord(y[i*32+0]) shl 0) or (ord(y[i*32+1]) shl 1) or (ord(y[i*32+2]) shl 2) or (ord(y[i*32+3]) shl 3) or (ord(y[i*32+4]) shl 4) or (ord(y[i*32+5]) shl 5) or (ord(y[i*32+6]) shl 6) or (ord(y[i*32+7]) shl 7);
  ey[1]:=(ord(y[i*32+8]) shl 0) or (ord(y[i*32+9]) shl 1) or (ord(y[i*32+10]) shl 2) or (ord(y[i*32+11]) shl 3) or (ord(y[i*32+12]) shl 4) or (ord(y[i*32+13]) shl 5) or (ord(y[i*32+14]) shl 6) or (ord(y[i*32+15]) shl 7);
  ey[3]:=(ord(y[i*32+16]) shl 0) or (ord(y[i*32+17]) shl 1) or (ord(y[i*32+18]) shl 2) or (ord(y[i*32+19]) shl 3) or (ord(y[i*32+20]) shl 4) or (ord(y[i*32+21]) shl 5) or (ord(y[i*32+22]) shl 6) or (ord(y[i*32+23]) shl 7);
  ey[4]:=(ord(y[i*32+24]) shl 0) or (ord(y[i*32+25]) shl 1) or (ord(y[i*32+26]) shl 2) or (ord(y[i*32+27]) shl 3) or (ord(y[i*32+28]) shl 4) or (ord(y[i*32+29]) shl 5) or (ord(y[i*32+30]) shl 6) or (ord(y[i*32+31]) shl 7);
  ex[2]:=ex[0] xor ex[1];
  ex[5]:=ex[3] xor ex[4];
  ex[6]:=ex[0] xor ex[3];
  ex[7]:=ex[1] xor ex[4];
  ...
  ey[7]:=ey[1] xor ey[4];
  ey[8]:=ey[2] xor ey[5];
  p[0]:=@mul8Bool[ix[0] or ex[0]];
  p[1]:=@mul8Bool[ix[1] or ey[0]];
  p[2]:=@mul8Bool[ix[2] or ex[1]];
  p[3]:=@mul8Bool[ix[3] or ey[1]];
  p[4]:=@mul8Bool[ix[4] or ex[2]];
  p[5]:=@mul8Bool[ix[5] or ey[2]];
  p[6]:=@mul8Bool[ix[6] or ex[3]];
  p[7]:=@mul8Bool[ix[7] or ey[3]];
  p[8]:=@mul8Bool[ix[8] or ex[4]];
  p[9]:=@mul8Bool[ix[9] or ey[4]];
  p[10]:=@mul8Bool[ix[10] or ex[5]];
  p[11]:=@mul8Bool[ix[11] or ey[5]];
  p[12]:=@mul8Bool[ix[12] or ex[6]];
  p[13]:=@mul8Bool[ix[13] or ey[6]];
  p[14]:=@mul8Bool[ix[14] or ex[7]];
  p[15]:=@mul8Bool[ix[15] or ey[7]];
  p[16]:=@mul8Bool[ix[16] or ex[8]];
  p[17]:=@mul8Bool[ix[17] or ey[8]];
  p[18]:=@mul8Bool[ix[18] or ex[0]];
  p[19]:=@mul8Bool[ix[19] or ey[0]];
  p[20]:=@mul8Bool[ix[20] or ex[1]];
  p[21]:=@mul8Bool[ix[21] or ey[1]];
  p[22]:=@mul8Bool[ix[22] or ex[2]];
  p[23]:=@mul8Bool[ix[23] or ey[2]];
  p[24]:=@mul8Bool[ix[24] or ex[3]];
  p[25]:=@mul8Bool[ix[25] or ey[3]];
  p[26]:=@mul8Bool[ix[26] or ex[4]];
  p[27]:=@mul8Bool[ix[27] or ey[4]];
  p[28]:=@mul8Bool[ix[28] or ex[5]];
  p[29]:=@mul8Bool[ix[29] or ey[5]];
  p[30]:=@mul8Bool[ix[30] or ex[6]];
  p[31]:=@mul8Bool[ix[31] or ey[6]];
  p[32]:=@mul8Bool[ix[32] or ex[7]];
  p[33]:=@mul8Bool[ix[33] or ey[7]];
  p[34]:=@mul8Bool[ix[34] or ex[8]];
  p[35]:=@mul8Bool[ix[35] or ey[8]];
    l0:=p[0]^[0] xor p[1]^[0]; l1:=p[0]^[0+8] xor p[1]^[0+8];
    h0:=p[2]^[0] xor p[3]^[0]; h1:=p[2]^[0+8] xor p[3]^[0+8];
    m0:=p[4]^[0] xor p[5]^[0]; m1:=p[4]^[0+8] xor p[5]^[0+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[0] xor p[7]^[0]; l1:=p[6]^[0+8] xor p[7]^[0+8];
    h0:=p[8]^[0] xor p[9]^[0]; h1:=p[8]^[0+8] xor p[9]^[0+8];
    m0:=p[10]^[0] xor p[11]^[0]; m1:=p[10]^[0+8] xor p[11]^[0+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[0] xor p[13]^[0]; l1:=p[12]^[0+8] xor p[13]^[0+8];
    h0:=p[14]^[0] xor p[15]^[0]; h1:=p[14]^[0+8] xor p[15]^[0+8];
    m0:=p[16]^[0] xor p[17]^[0]; m1:=p[16]^[0+8] xor p[17]^[0+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+0]:=z[0] xor cx[0+0];
    x[i*32+8+0]:=z[1] xor cx[8+0];
    x[i*32+16+0]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+0];
    x[i*32+24+0]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+0];
    cx[0+0]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+0]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+0]:=z[6];
    cx[24+0]:=z[7];
    l0:=p[18]^[0] xor p[19]^[0]; l1:=p[18]^[0+8] xor p[19]^[0+8];
    h0:=p[20]^[0] xor p[21]^[0]; h1:=p[20]^[0+8] xor p[21]^[0+8];
    m0:=p[22]^[0] xor p[23]^[0]; m1:=p[22]^[0+8] xor p[23]^[0+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[0] xor p[25]^[0]; l1:=p[24]^[0+8] xor p[25]^[0+8];
    h0:=p[26]^[0] xor p[27]^[0]; h1:=p[26]^[0+8] xor p[27]^[0+8];
    m0:=p[28]^[0] xor p[29]^[0]; m1:=p[28]^[0+8] xor p[29]^[0+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[0] xor p[31]^[0]; l1:=p[30]^[0+8] xor p[31]^[0+8];
    h0:=p[32]^[0] xor p[33]^[0]; h1:=p[32]^[0+8] xor p[33]^[0+8];
    m0:=p[34]^[0] xor p[35]^[0]; m1:=p[34]^[0+8] xor p[35]^[0+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+0]:=z[0] xor cy[0+0];
    y[i*32+8+0]:=z[1] xor cy[8+0];
    y[i*32+16+0]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+0];
    y[i*32+24+0]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+0];
    cy[0+0]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+0]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+0]:=z[6];
    cy[24+0]:=z[7];
    l0:=p[0]^[1] xor p[1]^[1]; l1:=p[0]^[1+8] xor p[1]^[1+8];
    h0:=p[2]^[1] xor p[3]^[1]; h1:=p[2]^[1+8] xor p[3]^[1+8];
    m0:=p[4]^[1] xor p[5]^[1]; m1:=p[4]^[1+8] xor p[5]^[1+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[1] xor p[7]^[1]; l1:=p[6]^[1+8] xor p[7]^[1+8];
    h0:=p[8]^[1] xor p[9]^[1]; h1:=p[8]^[1+8] xor p[9]^[1+8];
    m0:=p[10]^[1] xor p[11]^[1]; m1:=p[10]^[1+8] xor p[11]^[1+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[1] xor p[13]^[1]; l1:=p[12]^[1+8] xor p[13]^[1+8];
    h0:=p[14]^[1] xor p[15]^[1]; h1:=p[14]^[1+8] xor p[15]^[1+8];
    m0:=p[16]^[1] xor p[17]^[1]; m1:=p[16]^[1+8] xor p[17]^[1+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+1]:=z[0] xor cx[0+1];
    x[i*32+8+1]:=z[1] xor cx[8+1];
    x[i*32+16+1]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+1];
    x[i*32+24+1]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+1];
    cx[0+1]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+1]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+1]:=z[6];
    cx[24+1]:=z[7];
    l0:=p[18]^[1] xor p[19]^[1]; l1:=p[18]^[1+8] xor p[19]^[1+8];
    h0:=p[20]^[1] xor p[21]^[1]; h1:=p[20]^[1+8] xor p[21]^[1+8];
    m0:=p[22]^[1] xor p[23]^[1]; m1:=p[22]^[1+8] xor p[23]^[1+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[1] xor p[25]^[1]; l1:=p[24]^[1+8] xor p[25]^[1+8];
    h0:=p[26]^[1] xor p[27]^[1]; h1:=p[26]^[1+8] xor p[27]^[1+8];
    m0:=p[28]^[1] xor p[29]^[1]; m1:=p[28]^[1+8] xor p[29]^[1+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[1] xor p[31]^[1]; l1:=p[30]^[1+8] xor p[31]^[1+8];
    h0:=p[32]^[1] xor p[33]^[1]; h1:=p[32]^[1+8] xor p[33]^[1+8];
    m0:=p[34]^[1] xor p[35]^[1]; m1:=p[34]^[1+8] xor p[35]^[1+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+1]:=z[0] xor cy[0+1];
    y[i*32+8+1]:=z[1] xor cy[8+1];
    y[i*32+16+1]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+1];
    y[i*32+24+1]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+1];
    cy[0+1]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+1]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+1]:=z[6];
    cy[24+1]:=z[7];
    l0:=p[0]^[2] xor p[1]^[2]; l1:=p[0]^[2+8] xor p[1]^[2+8];
    h0:=p[2]^[2] xor p[3]^[2]; h1:=p[2]^[2+8] xor p[3]^[2+8];
    m0:=p[4]^[2] xor p[5]^[2]; m1:=p[4]^[2+8] xor p[5]^[2+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[2] xor p[7]^[2]; l1:=p[6]^[2+8] xor p[7]^[2+8];
    h0:=p[8]^[2] xor p[9]^[2]; h1:=p[8]^[2+8] xor p[9]^[2+8];
    m0:=p[10]^[2] xor p[11]^[2]; m1:=p[10]^[2+8] xor p[11]^[2+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[2] xor p[13]^[2]; l1:=p[12]^[2+8] xor p[13]^[2+8];
    h0:=p[14]^[2] xor p[15]^[2]; h1:=p[14]^[2+8] xor p[15]^[2+8];
    m0:=p[16]^[2] xor p[17]^[2]; m1:=p[16]^[2+8] xor p[17]^[2+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+2]:=z[0] xor cx[0+2];
    x[i*32+8+2]:=z[1] xor cx[8+2];
    x[i*32+16+2]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+2];
    x[i*32+24+2]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+2];
    cx[0+2]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+2]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+2]:=z[6];
    cx[24+2]:=z[7];
    l0:=p[18]^[2] xor p[19]^[2]; l1:=p[18]^[2+8] xor p[19]^[2+8];
    h0:=p[20]^[2] xor p[21]^[2]; h1:=p[20]^[2+8] xor p[21]^[2+8];
    m0:=p[22]^[2] xor p[23]^[2]; m1:=p[22]^[2+8] xor p[23]^[2+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[2] xor p[25]^[2]; l1:=p[24]^[2+8] xor p[25]^[2+8];
    h0:=p[26]^[2] xor p[27]^[2]; h1:=p[26]^[2+8] xor p[27]^[2+8];
    m0:=p[28]^[2] xor p[29]^[2]; m1:=p[28]^[2+8] xor p[29]^[2+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[2] xor p[31]^[2]; l1:=p[30]^[2+8] xor p[31]^[2+8];
    h0:=p[32]^[2] xor p[33]^[2]; h1:=p[32]^[2+8] xor p[33]^[2+8];
    m0:=p[34]^[2] xor p[35]^[2]; m1:=p[34]^[2+8] xor p[35]^[2+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+2]:=z[0] xor cy[0+2];
    y[i*32+8+2]:=z[1] xor cy[8+2];
    y[i*32+16+2]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+2];
    y[i*32+24+2]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+2];
    cy[0+2]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+2]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+2]:=z[6];
    cy[24+2]:=z[7];
    l0:=p[0]^[3] xor p[1]^[3]; l1:=p[0]^[3+8] xor p[1]^[3+8];
    h0:=p[2]^[3] xor p[3]^[3]; h1:=p[2]^[3+8] xor p[3]^[3+8];
    m0:=p[4]^[3] xor p[5]^[3]; m1:=p[4]^[3+8] xor p[5]^[3+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[3] xor p[7]^[3]; l1:=p[6]^[3+8] xor p[7]^[3+8];
    h0:=p[8]^[3] xor p[9]^[3]; h1:=p[8]^[3+8] xor p[9]^[3+8];
    m0:=p[10]^[3] xor p[11]^[3]; m1:=p[10]^[3+8] xor p[11]^[3+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[3] xor p[13]^[3]; l1:=p[12]^[3+8] xor p[13]^[3+8];
    h0:=p[14]^[3] xor p[15]^[3]; h1:=p[14]^[3+8] xor p[15]^[3+8];
    m0:=p[16]^[3] xor p[17]^[3]; m1:=p[16]^[3+8] xor p[17]^[3+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+3]:=z[0] xor cx[0+3];
    x[i*32+8+3]:=z[1] xor cx[8+3];
    x[i*32+16+3]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+3];
    x[i*32+24+3]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+3];
    cx[0+3]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+3]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+3]:=z[6];
    cx[24+3]:=z[7];
    l0:=p[18]^[3] xor p[19]^[3]; l1:=p[18]^[3+8] xor p[19]^[3+8];
    h0:=p[20]^[3] xor p[21]^[3]; h1:=p[20]^[3+8] xor p[21]^[3+8];
    m0:=p[22]^[3] xor p[23]^[3]; m1:=p[22]^[3+8] xor p[23]^[3+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[3] xor p[25]^[3]; l1:=p[24]^[3+8] xor p[25]^[3+8];
    h0:=p[26]^[3] xor p[27]^[3]; h1:=p[26]^[3+8] xor p[27]^[3+8];
    m0:=p[28]^[3] xor p[29]^[3]; m1:=p[28]^[3+8] xor p[29]^[3+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[3] xor p[31]^[3]; l1:=p[30]^[3+8] xor p[31]^[3+8];
    h0:=p[32]^[3] xor p[33]^[3]; h1:=p[32]^[3+8] xor p[33]^[3+8];
    m0:=p[34]^[3] xor p[35]^[3]; m1:=p[34]^[3+8] xor p[35]^[3+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+3]:=z[0] xor cy[0+3];
    y[i*32+8+3]:=z[1] xor cy[8+3];
    y[i*32+16+3]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+3];
    y[i*32+24+3]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+3];
    cy[0+3]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+3]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+3]:=z[6];
    cy[24+3]:=z[7];
    l0:=p[0]^[4] xor p[1]^[4]; l1:=p[0]^[4+8] xor p[1]^[4+8];
    h0:=p[2]^[4] xor p[3]^[4]; h1:=p[2]^[4+8] xor p[3]^[4+8];
    m0:=p[4]^[4] xor p[5]^[4]; m1:=p[4]^[4+8] xor p[5]^[4+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[4] xor p[7]^[4]; l1:=p[6]^[4+8] xor p[7]^[4+8];
    h0:=p[8]^[4] xor p[9]^[4]; h1:=p[8]^[4+8] xor p[9]^[4+8];
    m0:=p[10]^[4] xor p[11]^[4]; m1:=p[10]^[4+8] xor p[11]^[4+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[4] xor p[13]^[4]; l1:=p[12]^[4+8] xor p[13]^[4+8];
    h0:=p[14]^[4] xor p[15]^[4]; h1:=p[14]^[4+8] xor p[15]^[4+8];
    m0:=p[16]^[4] xor p[17]^[4]; m1:=p[16]^[4+8] xor p[17]^[4+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+4]:=z[0] xor cx[0+4];
    x[i*32+8+4]:=z[1] xor cx[8+4];
    x[i*32+16+4]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+4];
    x[i*32+24+4]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+4];
    cx[0+4]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+4]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+4]:=z[6];
    cx[24+4]:=z[7];
    l0:=p[18]^[4] xor p[19]^[4]; l1:=p[18]^[4+8] xor p[19]^[4+8];
    h0:=p[20]^[4] xor p[21]^[4]; h1:=p[20]^[4+8] xor p[21]^[4+8];
    m0:=p[22]^[4] xor p[23]^[4]; m1:=p[22]^[4+8] xor p[23]^[4+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[4] xor p[25]^[4]; l1:=p[24]^[4+8] xor p[25]^[4+8];
    h0:=p[26]^[4] xor p[27]^[4]; h1:=p[26]^[4+8] xor p[27]^[4+8];
    m0:=p[28]^[4] xor p[29]^[4]; m1:=p[28]^[4+8] xor p[29]^[4+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[4] xor p[31]^[4]; l1:=p[30]^[4+8] xor p[31]^[4+8];
    h0:=p[32]^[4] xor p[33]^[4]; h1:=p[32]^[4+8] xor p[33]^[4+8];
    m0:=p[34]^[4] xor p[35]^[4]; m1:=p[34]^[4+8] xor p[35]^[4+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+4]:=z[0] xor cy[0+4];
    y[i*32+8+4]:=z[1] xor cy[8+4];
    y[i*32+16+4]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+4];
    y[i*32+24+4]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+4];
    cy[0+4]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+4]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+4]:=z[6];
    cy[24+4]:=z[7];
    l0:=p[0]^[5] xor p[1]^[5]; l1:=p[0]^[5+8] xor p[1]^[5+8];
    h0:=p[2]^[5] xor p[3]^[5]; h1:=p[2]^[5+8] xor p[3]^[5+8];
    m0:=p[4]^[5] xor p[5]^[5]; m1:=p[4]^[5+8] xor p[5]^[5+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[5] xor p[7]^[5]; l1:=p[6]^[5+8] xor p[7]^[5+8];
    h0:=p[8]^[5] xor p[9]^[5]; h1:=p[8]^[5+8] xor p[9]^[5+8];
    m0:=p[10]^[5] xor p[11]^[5]; m1:=p[10]^[5+8] xor p[11]^[5+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[5] xor p[13]^[5]; l1:=p[12]^[5+8] xor p[13]^[5+8];
    h0:=p[14]^[5] xor p[15]^[5]; h1:=p[14]^[5+8] xor p[15]^[5+8];
    m0:=p[16]^[5] xor p[17]^[5]; m1:=p[16]^[5+8] xor p[17]^[5+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+5]:=z[0] xor cx[0+5];
    x[i*32+8+5]:=z[1] xor cx[8+5];
    x[i*32+16+5]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+5];
    x[i*32+24+5]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+5];
    cx[0+5]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+5]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+5]:=z[6];
    cx[24+5]:=z[7];
    l0:=p[18]^[5] xor p[19]^[5]; l1:=p[18]^[5+8] xor p[19]^[5+8];
    h0:=p[20]^[5] xor p[21]^[5]; h1:=p[20]^[5+8] xor p[21]^[5+8];
    m0:=p[22]^[5] xor p[23]^[5]; m1:=p[22]^[5+8] xor p[23]^[5+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[5] xor p[25]^[5]; l1:=p[24]^[5+8] xor p[25]^[5+8];
    h0:=p[26]^[5] xor p[27]^[5]; h1:=p[26]^[5+8] xor p[27]^[5+8];
    m0:=p[28]^[5] xor p[29]^[5]; m1:=p[28]^[5+8] xor p[29]^[5+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[5] xor p[31]^[5]; l1:=p[30]^[5+8] xor p[31]^[5+8];
    h0:=p[32]^[5] xor p[33]^[5]; h1:=p[32]^[5+8] xor p[33]^[5+8];
    m0:=p[34]^[5] xor p[35]^[5]; m1:=p[34]^[5+8] xor p[35]^[5+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+5]:=z[0] xor cy[0+5];
    y[i*32+8+5]:=z[1] xor cy[8+5];
    y[i*32+16+5]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+5];
    y[i*32+24+5]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+5];
    cy[0+5]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+5]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+5]:=z[6];
    cy[24+5]:=z[7];
    l0:=p[0]^[6] xor p[1]^[6]; l1:=p[0]^[6+8] xor p[1]^[6+8];
    h0:=p[2]^[6] xor p[3]^[6]; h1:=p[2]^[6+8] xor p[3]^[6+8];
    m0:=p[4]^[6] xor p[5]^[6]; m1:=p[4]^[6+8] xor p[5]^[6+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[6] xor p[7]^[6]; l1:=p[6]^[6+8] xor p[7]^[6+8];
    h0:=p[8]^[6] xor p[9]^[6]; h1:=p[8]^[6+8] xor p[9]^[6+8];
    m0:=p[10]^[6] xor p[11]^[6]; m1:=p[10]^[6+8] xor p[11]^[6+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[6] xor p[13]^[6]; l1:=p[12]^[6+8] xor p[13]^[6+8];
    h0:=p[14]^[6] xor p[15]^[6]; h1:=p[14]^[6+8] xor p[15]^[6+8];
    m0:=p[16]^[6] xor p[17]^[6]; m1:=p[16]^[6+8] xor p[17]^[6+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+6]:=z[0] xor cx[0+6];
    x[i*32+8+6]:=z[1] xor cx[8+6];
    x[i*32+16+6]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+6];
    x[i*32+24+6]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+6];
    cx[0+6]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+6]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+6]:=z[6];
    cx[24+6]:=z[7];
    l0:=p[18]^[6] xor p[19]^[6]; l1:=p[18]^[6+8] xor p[19]^[6+8];
    h0:=p[20]^[6] xor p[21]^[6]; h1:=p[20]^[6+8] xor p[21]^[6+8];
    m0:=p[22]^[6] xor p[23]^[6]; m1:=p[22]^[6+8] xor p[23]^[6+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[6] xor p[25]^[6]; l1:=p[24]^[6+8] xor p[25]^[6+8];
    h0:=p[26]^[6] xor p[27]^[6]; h1:=p[26]^[6+8] xor p[27]^[6+8];
    m0:=p[28]^[6] xor p[29]^[6]; m1:=p[28]^[6+8] xor p[29]^[6+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[6] xor p[31]^[6]; l1:=p[30]^[6+8] xor p[31]^[6+8];
    h0:=p[32]^[6] xor p[33]^[6]; h1:=p[32]^[6+8] xor p[33]^[6+8];
    m0:=p[34]^[6] xor p[35]^[6]; m1:=p[34]^[6+8] xor p[35]^[6+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+6]:=z[0] xor cy[0+6];
    y[i*32+8+6]:=z[1] xor cy[8+6];
    y[i*32+16+6]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+6];
    y[i*32+24+6]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+6];
    cy[0+6]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+6]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+6]:=z[6];
    cy[24+6]:=z[7];
    l0:=p[0]^[7] xor p[1]^[7]; l1:=p[0]^[7+8] xor p[1]^[7+8];
    h0:=p[2]^[7] xor p[3]^[7]; h1:=p[2]^[7+8] xor p[3]^[7+8];
    m0:=p[4]^[7] xor p[5]^[7]; m1:=p[4]^[7+8] xor p[5]^[7+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[6]^[7] xor p[7]^[7]; l1:=p[6]^[7+8] xor p[7]^[7+8];
    h0:=p[8]^[7] xor p[9]^[7]; h1:=p[8]^[7+8] xor p[9]^[7+8];
    m0:=p[10]^[7] xor p[11]^[7]; m1:=p[10]^[7+8] xor p[11]^[7+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[12]^[7] xor p[13]^[7]; l1:=p[12]^[7+8] xor p[13]^[7+8];
    h0:=p[14]^[7] xor p[15]^[7]; h1:=p[14]^[7+8] xor p[15]^[7+8];
    m0:=p[16]^[7] xor p[17]^[7]; m1:=p[16]^[7+8] xor p[17]^[7+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    x[i*32+0+7]:=z[0] xor cx[0+7];
    x[i*32+8+7]:=z[1] xor cx[8+7];
    x[i*32+16+7]:=z[2] xor z[0] xor z[4] xor z[8] xor cx[16+7];
    x[i*32+24+7]:=z[3] xor z[1] xor z[5] xor z[9] xor cx[24+7];
    cx[0+7]:=z[4] xor z[2] xor z[6] xor z[10];
    cx[8+7]:=z[5] xor z[3] xor z[7] xor z[11];
    cx[16+7]:=z[6];
    cx[24+7]:=z[7];
    l0:=p[18]^[7] xor p[19]^[7]; l1:=p[18]^[7+8] xor p[19]^[7+8];
    h0:=p[20]^[7] xor p[21]^[7]; h1:=p[20]^[7+8] xor p[21]^[7+8];
    m0:=p[22]^[7] xor p[23]^[7]; m1:=p[22]^[7+8] xor p[23]^[7+8];
    z[0]:=l0; z[1]:=l1 xor l0 xor m0 xor h0;
    z[2]:=h0 xor l1 xor m1 xor h1; z[3]:=h1;
    l0:=p[24]^[7] xor p[25]^[7]; l1:=p[24]^[7+8] xor p[25]^[7+8];
    h0:=p[26]^[7] xor p[27]^[7]; h1:=p[26]^[7+8] xor p[27]^[7+8];
    m0:=p[28]^[7] xor p[29]^[7]; m1:=p[28]^[7+8] xor p[29]^[7+8];
    z[4]:=l0; z[5]:=l1 xor l0 xor m0 xor h0;
    z[6]:=h0 xor l1 xor m1 xor h1; z[7]:=h1;
    l0:=p[30]^[7] xor p[31]^[7]; l1:=p[30]^[7+8] xor p[31]^[7+8];
    h0:=p[32]^[7] xor p[33]^[7]; h1:=p[32]^[7+8] xor p[33]^[7+8];
    m0:=p[34]^[7] xor p[35]^[7]; m1:=p[34]^[7+8] xor p[35]^[7+8];
    z[8]:=l0; z[9]:=l1 xor l0 xor m0 xor h0;
    z[10]:=h0 xor l1 xor m1 xor h1; z[11]:=h1;
    y[i*32+0+7]:=z[0] xor cy[0+7];
    y[i*32+8+7]:=z[1] xor cy[8+7];
    y[i*32+16+7]:=z[2] xor z[0] xor z[4] xor z[8] xor cy[16+7];
    y[i*32+24+7]:=z[3] xor z[1] xor z[5] xor z[9] xor cy[24+7];
    cy[0+7]:=z[4] xor z[2] xor z[6] xor z[10];
    cy[8+7]:=z[5] xor z[3] xor z[7] xor z[11];
    cy[16+7]:=z[6];
    cy[24+7]:=z[7];
  end;
...
for k:=0 to 31 do
  begin
  c00:=c00 or (LongWord(ord(m00[k])) shl k); c01:=c01 or (LongWord(ord(m01[k])) shl k);
  c10:=c10 or (LongWord(ord(m10[k])) shl k); c11:=c11 or (LongWord(ord(m11[k])) shl k);
  end;
ix[0]:=(((c00 shr 0)) and $FF) shl 8;
ix[2]:=(((c00 shr 8)) and $FF) shl 8;
ix[4]:=(((c00 shr 0) xor (c00 shr 8)) and $FF) shl 8;
ix[6]:=(((c00 shr 16)) and $FF) shl 8;
ix[8]:=(((c00 shr 24)) and $FF) shl 8;
ix[10]:=(((c00 shr 16) xor (c00 shr 24)) and $FF) shl 8;
ix[12]:=(((c00 shr 0) xor (c00 shr 16)) and $FF) shl 8;
ix[14]:=(((c00 shr 8) xor (c00 shr 24)) and $FF) shl 8;
ix[16]:=(((c00 shr 0) xor (c00 shr 8) xor (c00 shr 16) xor (c00 shr 24)) and $FF) shl 8;
ix[1]:=(((c01 shr 0)) and $FF) shl 8;
ix[3]:=(((c01 shr 8)) and $FF) shl 8;
ix[5]:=(((c01 shr 0) xor (c01 shr 8)) and $FF) shl 8;
ix[7]:=(((c01 shr 16)) and $FF) shl 8;
ix[9]:=(((c01 shr 24)) and $FF) shl 8;
ix[11]:=(((c01 shr 16) xor (c01 shr 24)) and $FF) shl 8;
ix[13]:=(((c01 shr 0) xor (c01 shr 16)) and $FF) shl 8;
ix[15]:=(((c01 shr 8) xor (c01 shr 24)) and $FF) shl 8;
ix[17]:=(((c01 shr 0) xor (c01 shr 8) xor (c01 shr 16) xor (c01 shr 24)) and $FF) shl 8;
ix[18]:=(((c10 shr 0)) and $FF) shl 8;
ix[20]:=(((c10 shr 8)) and $FF) shl 8;
ix[22]:=(((c10 shr 0) xor (c10 shr 8)) and $FF) shl 8;
ix[24]:=(((c10 shr 16)) and $FF) shl 8;
ix[26]:=(((c10 shr 24)) and $FF) shl 8;
ix[28]:=(((c10 shr 16) xor (c10 shr 24)) and $FF) shl 8;
ix[30]:=(((c10 shr 0) xor (c10 shr 16)) and $FF) shl 8;
ix[32]:=(((c10 shr 8) xor (c10 shr 24)) and $FF) shl 8;
ix[34]:=(((c10 shr 0) xor (c10 shr 8) xor (c10 shr 16) xor (c10 shr 24)) and $FF) shl 8;
ix[19]:=(((c11 shr 0)) and $FF) shl 8;
ix[21]:=(((c11 shr 8)) and $FF) shl 8;
ix[23]:=(((c11 shr 0) xor (c11 shr 8)) and $FF) shl 8;
ix[25]:=(((c11 shr 16)) and $FF) shl 8;
ix[27]:=(((c11 shr 24)) and $FF) shl 8;
ix[29]:=(((c11 shr 16) xor (c11 shr 24)) and $FF) shl 8;
ix[31]:=(((c11 shr 0) xor (c11 shr 16)) and $FF) shl 8;
ix[33]:=(((c11 shr 8) xor (c11 shr 24)) and $FF) shl 8;
ix[35]:=(((c11 shr 0) xor (c11 shr 8) xor (c11 shr 16) xor (c11 shr 24)) and $FF) shl 8;
...
begin
TimeMark('g');
ApplyFastUCombo(f,c,x,t,n-1,longint(n div 2),2);
GeneMat:=true;
