...
{ H79: use fourteen base products for 160-coefficient leaves by reusing a diagonal. }
...
{ H79: split 160 coefficients into 64+96 and reuse the common high diagonal. }
...
for i:=0 to 31 do begin ax[i]:=a[ao+0+i] xor a[ao+64+i]; bx[i]:=b[bo+0+i] xor b[bo+64+i]; end;
CLMul32Bool(ax,0,32,bx,0,32,g0,true);
for i:=0 to 31 do begin ax[i]:=a[ao+32+i] xor a[ao+96+i]; bx[i]:=b[bo+32+i] xor b[bo+96+i]; end;
CLMul32Bool(ax,0,32,bx,0,32,g1,true);
for i:=0 to 31 do begin ax[i]:=a[ao+0+i] xor a[ao+32+i] xor a[ao+64+i] xor a[ao+96+i]; bx[i]:=b[bo+0+i] xor b[bo+32+i] xor b[bo+64+i] xor b[bo+96+i]; end;
CLMul32Bool(ax,0,32,bx,0,32,g01,true);
for i:=0 to 31 do begin ax[i]:=a[ao+0+i] xor a[ao+64+i] xor at[i]; bx[i]:=b[bo+0+i] xor b[bo+64+i] xor bt[i]; end;
CLMul32Bool(ax,0,32,bx,0,32,g02,true);
for i:=0 to 31 do begin ax[i]:=a[ao+32+i] xor a[ao+96+i] xor at[i]; bx[i]:=b[bo+32+i] xor b[bo+96+i] xor bt[i]; end;
CLMul32Bool(ax,0,32,bx,0,32,g12,true);
for i:=0 to 63 do
  begin
  e1[i]:=e1[i] xor e0[i] xor e2[i];
  f01[i]:=f01[i] xor f0[i] xor f1[i];
  f02[i]:=f02[i] xor f0[i] xor f1[i] xor f2[i];
  f12[i]:=f12[i] xor f1[i] xor f2[i];
  g01[i]:=g01[i] xor g0[i] xor g1[i];
  g02[i]:=g02[i] xor g0[i] xor g1[i] xor f2[i];
  g12[i]:=g12[i] xor g1[i] xor f2[i];
  g0[i]:=e2[i] xor e0[i] xor f0[i] xor g0[i];
  g01[i]:=e1[i] xor f01[i] xor g01[i];
  g02[i]:=e2[i] xor f02[i] xor g02[i] xor f0[i];
  g12[i]:=f12[i] xor g12[i] xor f01[i];
  end;
for i:=0 to 31 do
  begin
