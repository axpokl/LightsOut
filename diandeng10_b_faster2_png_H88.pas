...
var xx,yy:QWord; cx,cy:LongWord;
begin
cx:=0; cy:=0;
lim:=dx; if dy>lim then lim:=dy; lim:=(lim+32) shr 5;
for i:=0 to lim-1 do
  begin
  index[0]:=((x[i] shr 0) and 7) or (((y[i] shr 0) and 7) shl 3);
  index[1]:=((x[i] shr 3) and 7) or (((y[i] shr 3) and 7) shl 3);
  index[2]:=((x[i] shr 6) and 7) or (((y[i] shr 6) and 7) shl 3);
  index[3]:=((x[i] shr 9) and 7) or (((y[i] shr 9) and 7) shl 3);
  index[4]:=((x[i] shr 12) and 7) or (((y[i] shr 12) and 7) shl 3);
  index[5]:=((x[i] shr 15) and 7) or (((y[i] shr 15) and 7) shl 3);
  index[6]:=((x[i] shr 18) and 7) or (((y[i] shr 18) and 7) shl 3);
  index[7]:=((x[i] shr 21) and 7) or (((y[i] shr 21) and 7) shl 3);
  index[8]:=((x[i] shr 24) and 7) or (((y[i] shr 24) and 7) shl 3);
  index[9]:=((x[i] shr 27) and 7) or (((y[i] shr 27) and 7) shl 3);
  index[10]:=((x[i] shr 30) and 3) or (((y[i] shr 30) and 3) shl 3);
...
var half,period,words,mask,shift,left,right,p,take:longint;
var value,last:LongWord;
function ReadCycle(const v:TWordArray; pos:longint):LongWord; inline;
begin
if pos<half then
  begin
...
begin
half:=hi+2; period:=half*2; words:=(half+32) shr 5;
...
while p<=hi do
  begin
  take:=hi-p+1; if take>32 then take:=32;
...
while mask>0 do
  begin
  shift:=mask mod period; if shift>half then shift:=period-shift; left:=period-shift; if left=period then left:=0;
  right:=shift;
  if summed then
    begin
    { S_(2k)=J*S_k^2+F_(k-1)^2; S_(2k+1)=J*S_k^2+F_k^2. }
    if (degree and mask)<>0 then
    for p:=0 to words-1 do
      begin
      ts[p]:=ss[p] xor ReadCycle(ss,left) xor ReadCycle(ss,right) xor a[p];
      inc(left,32); if left>=period then left:=left mod period;
      inc(right,32); if right>=period then right:=right mod period;
      end
    else
    for p:=0 to words-1 do
      begin
      ts[p]:=ss[p] xor ReadCycle(ss,left) xor ReadCycle(ss,right) xor b[p];
      inc(left,32); if left>=period then left:=left mod period;
      inc(right,32); if right>=period then right:=right mod period;
      end;
    ...
    left:=period-shift; if left=period then left:=0; right:=shift;
    end;
  if (degree and mask)<>0 then
    begin
    for p:=0 to words-1 do
      begin
      t[p]:=a[p] xor ReadCycle(a,left) xor ReadCycle(a,right); b[p]:=a[p] xor b[p];
      inc(left,32); if left>=period then left:=left mod period;
      inc(right,32); if right>=period then right:=right mod period;
      end;
    ...
    for p:=0 to words-1 do
      begin
      t[p]:=b[p] xor ReadCycle(b,left) xor ReadCycle(b,right); a[p]:=a[p] xor b[p];
      inc(left,32); if left>=period then left:=left mod period;
      inc(right,32); if right>=period then right:=right mod period;
      end;
    t[words-1]:=t[words-1] and last;
...
if summed then a:=ss;
VecZero(dst);
for p:=0 to (hi+32) shr 5-1 do dst[p]:=ReadDyn32(a,p*32+1);
MaskDeg(dst,hi);
end;
...
var j,sh:longint; v:LongWord;
begin
if gap>=len then exit;
if gap<32 then
  for j:=0 to (len-1) shr 5 do
    begin
    v:=rem[j];
    if j>0 then v:=v xor (rem[j-1] shr (32-gap));
    sh:=gap;
    while sh<32 do begin v:=v xor (v shl sh); sh:=sh shl 1; end;
    rem[j]:=v;
    end
else
  for j:=gap shr 5 to (len-1) shr 5 do
    rem[j]:=rem[j] xor ReadVec32Any(rem,j*32-gap);
end;
procedure MultiplyBinomial(gap:longint);
var j:longint;
begin
for j:=(len-1) shr 5 downto gap shr 5 do
  rem[j]:=rem[j] xor ReadVec32Any(rem,j*32-gap);
end;
begin
