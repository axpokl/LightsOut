...
var half,period,words,mask,shift,p,take:longint;
...
function ReadPair(const v:TWordArray; p:longint):LongWord; inline;
var left,right:longint;
begin
if (p>=innerFirst) and (p<=innerLast) then
  begin
  if bitShift=0 then ReadPair:=v[p-wordShift] xor v[p+wordShift]
  else ReadPair:=(v[p-wordShift-1] shr (32-bitShift)) xor (v[p-wordShift] shl bitShift)
             xor (v[p+wordShift] shr bitShift) xor (v[p+wordShift+1] shl (32-bitShift));
  end
else
  begin
  left:=p*32-shift; if left<0 then inc(left,period);
  right:=p*32+shift; if right>=period then dec(right,period);
  ReadPair:=ReadCycle(v,left) xor ReadCycle(v,right);
  end;
end;
begin
...
while mask>0 do
  ...
  if summed then
    ...
    for p:=0 to words-1 do
      begin
      ts[p]:=ss[p] xor ReadPair(ss,p) xor a[p];
      end
    else
    for p:=0 to words-1 do
      begin
      ts[p]:=ss[p] xor ReadPair(ss,p) xor b[p];
      end;
  ...
  if (degree and mask)<>0 then
    ...
    for p:=0 to words-1 do
      begin
      t[p]:=a[p] xor ReadPair(a,p); b[p]:=a[p] xor b[p];
      end;
    ...
    for p:=0 to words-1 do
      begin
      t[p]:=b[p] xor ReadPair(b,p); a[p]:=a[p] xor b[p];
      end;
    t[words-1]:=t[words-1] and last;
    tmp:=b; b:=t; t:=tmp;
    end;
