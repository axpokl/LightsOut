...
const cut=8;
var afull,bfull,startB,tail:longint;
...
if lenWords<=cut then
  begin
  MulBaseBool(a,ao,alen,b,bo,blen,r,ro,alen+blen-1);
  exit;
  end;
hWords:=(lenWords+1) shr 1;
gWords:=lenWords-hWords;
...
if lenWords<=cut then
  begin
  MulBasePairBool(a,ao,alen,b,bo,blen,c,co,clen,r,ro,s,so,lenBits*2);
  exit;
  end;
hWords:=(lenWords+1) shr 1;
gWords:=lenWords-hWords;
...
if blockLen=32 then
  ...
  while pos<len do
    ...
    CLMul32Bool(rem.v^,pos,k,iv.v^,0,iv.d+1,qblock);
    lim:=(den.d+32) div 32-1;
    if lim>=(len-pos+31) div 32 then lim:=(len-pos+31) div 32-1;
    for j0:=0 to lim do
      begin
      CLMul32Bool(den.v^,j0*32,den.d+1-j0*32,qblock,0,k,product);
      for j1:=0 to 31 do
        begin
