...
var k2,d,j2:longint;
begin
for k2:=-2 to hi+1 do begin cur0[k2]:=false; cur1[k2]:=false; hcur[k2]:=false; vdst[k2]:=false; end;
for k2:=0 to hi do cur0[k2]:=vsrc[k2];
d:=PolyDeg(va,degmax);
if d<0 then exit;
pcur:=@cur0;
...
for j2:=0 to d do
  ...
  if j2>=d then break;
  pcur^[-1]:=false; pcur^[hi+1]:=false;
  hcur[-1]:=false; hcur[hi+1]:=false;
  for k2:=0 to hi do hcur[k2]:=pcur^[k2-1] xor pcur^[k2+1];
  for k2:=0 to hi do pnxt^[k2]:=hcur[k2-1] xor hcur[k2] xor hcur[k2+1];
  pnxt^[-1]:=false; pnxt^[hi+1]:=false;
...
for j2:=0 to d do
  begin
  if vv[j2] then for k2:=0 to hi do vdst[k2]:=vdst[k2] xor pcur^[k2];
  pcur^[-1]:=false; pcur^[hi+1]:=false;
  hcur[-1]:=false; hcur[hi+1]:=false;
  for k2:=0 to hi do hcur[k2]:=pcur^[k2-1] xor pcur^[k2+1];
  if vu[j2] then for k2:=0 to hi do vdst[k2]:=vdst[k2] xor hcur[k2];
  if j2>=d then break;
  for k2:=0 to hi do pnxt^[k2]:=hcur[k2-1] xor hcur[k2] xor hcur[k2+1];
  pnxt^[-1]:=false; pnxt^[hi+1]:=false;
  pt:=pcur; pcur:=pnxt; pnxt:=pt;
...
for j2:=0 to d do
  ...
  if vb[j2] then
    ...
    for k2:=0 to hi do vdst[k2]:=vdst[k2] xor hcur[k2];
    end;
  if j2>=d then break;
  for k2:=0 to hi do pnxt^[k2]:=hcur[k2-1] xor hcur[k2] xor hcur[k2+1];
  pnxt^[-1]:=false; pnxt^[hi+1]:=false;
