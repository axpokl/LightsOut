...
procedure StepJ(var dst:TVec; const src:TVec; l,r,hi:longint; var l2,r2:longint);
begin
VecStepRange(v1,src,l,r,hi);
l2:=l-1; if l2<0 then l2:=0;
r2:=r+1; if r2>hi then r2:=hi;
VecStepRange(dst,v1,l2,r2,hi);
VecXorRange(dst,v1,l2,r2);
l2:=l2-1; if l2<0 then l2:=0;
r2:=r2+1; if r2>hi then r2:=hi;
end;
...
var pcur,pnxt,pt:PVec;
var d,du,dv,j2,l,r,lh,rh,l2,r2:longint;
begin
...
for j2:=0 to d do
  begin
  if GetBit(vv,j2)<>0 then VecXorRange(vdst,pcur^,l,r);
  VecStepRange(hcur,pcur^,l,r,hi);
  lh:=l-1; if lh<0 then lh:=0;
  rh:=r+1; if rh>hi then rh:=hi;
  if GetBit(vu,j2)<>0 then VecXorRange(vdst,hcur,lh,rh);
  if j2>=d then break;
  VecStepRange(pnxt^,hcur,lh,rh,hi);
  VecXorRange(pnxt^,hcur,lh,rh);
  l2:=lh-1; if l2<0 then l2:=0;
  r2:=rh+1; if r2>hi then r2:=hi;
  l:=FirstBitRange(pnxt^,l2,r2);
  if l<0 then break;
  r:=LastBitRange(pnxt^,l,r2);
  pt:=pcur; pcur:=pnxt; pnxt:=pt;
...
var cur0,cur1,hcur:TVec;
var pcur,pnxt,pt:PVec;
var d,da,db,j2,l,r,lh,rh,l2,r2:longint;
begin
VecZero(cur0); VecZero(cur1); VecZero(hcur); VecZero(vdst);
VecCopy(cur0,vsrc);
MaskDeg(cur0,hi);
...
for j2:=0 to d do
  ...
  if GetBit(vb,j2)<>0 then
    begin
    VecXorRange(vdst,pcur^,l,r);
    VecXorRange(vdst,hcur,lh,rh);
    end;
  if j2>=d then break;
  VecStepRange(pnxt^,hcur,lh,rh,hi);
  VecXorRange(pnxt^,hcur,lh,rh);
