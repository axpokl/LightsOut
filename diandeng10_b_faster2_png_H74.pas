...
{ H74: linear Fibonacci half-kernels for y construction and verification. }
...
const cut=8;
var afull,bfull,startB,ai,bi,ri:longint;
...
if lenWords<=cut then
  ...
  for i0:=0 to aWords-1 do if a[ao+i0]<>0 then
    ...
    for j0:=0 to last-1 do if b[bo+j0]<>0 then
      begin
      product:=CLMul32(a[ao+i0],b[bo+j0]);
...
procedure KarRecPairW(const a:TWordArray; ao:longint;
                      ...
                      var work:TWordArray; wo:longint);
const cut=8;
var i0,j0,lenBits,hWords,gWords,ax0,bx0,cx0,zr0,zs0,rec0:longint;
...
if lenWords<=8 then
  ...
  for i0:=0 to aWords-1 do if a[ao+i0]<>0 then
    ...
    for j0:=0 to common-1 do
      ...
      if c[co+j0]<>0 then
        begin
        product:=CLMul32(a[ao+i0],c[co+j0]);
...
if (alen<32) or (blen<32) or (lim<=256) then
  ...
  HPWAlloc(c,lenWords);
  for i0:=0 to aWords-1 do if a.v^[i0]<>0 then
    begin
    last:=lenWords-i0; if last>bWords then last:=bWords;
    for j0:=0 to last-1 do if b.v^[j0]<>0 then
      begin
      product:=CLMul32(a.v^[i0],b.v^[j0]);
...
if count=1024 then
  begin
  UKernel1024Upper(va,first,degmax,aa);
  UKernel1024Upper(vb,first,degmax,bb);
  w0:=dstBit shr 5;
  for k0:=0 to 31 do
    begin
    if mode=0 then
      sa1:=(bb[31+k0] shr 62) xor (bb[32+k0] shl 2) xor
           (aa[31+k0] shr 63) xor (aa[32+k0] shl 1) xor
           (aa[31+k0] shr 61) xor (aa[32+k0] shl 3)
    else
      sa1:=(aa[31+k0] shr 62) xor (aa[32+k0] shl 2) xor
           (bb[31+k0] shr 63) xor (bb[32+k0] shl 1) xor
           (bb[31+k0] shr 62) xor (bb[32+k0] shl 2) xor
           (bb[31+k0] shr 61) xor (bb[32+k0] shl 3);
    dst[w0+(k0 shl 1)]:=LongWord(sa1);
    dst[w0+(k0 shl 1)+1]:=LongWord(sa1 shr 32);
    end;
  exit;
  end;
...
{ H74: the same Fibonacci doubling of nonnegative Laurent offsets. }
procedure BuildFibonacciHalfKernelW(degree:longint; var r:TWordArray; words:longint; summed:boolean=false);
var a,b,fa,fb,tmp:TWordArray;
var mask,cur,next,p,dst:longint;
var odd:boolean; av,bv,an,bn,carry,value:LongWord; az,bz,asum,bsum:QWord;
begin
...
while mask>0 do
  ...
  for p:=0 to cur shr 5 do
    begin
    av:=a[p]; bv:=b[p]; an:=0; bn:=0;
    if p+1<words then begin an:=a[p+1]; bn:=b[p+1]; end;
    az:=SpreadBits32(av); bz:=SpreadBits32(bv);
    asum:=(az shl 1) xor (az shr 1) xor (QWord(an and 1) shl 63);
    bsum:=(bz shl 1) xor (bz shr 1) xor (QWord(bn and 1) shl 63);
    if odd then begin bz:=az xor bz; az:=az xor asum; end
    else begin az:=az xor bz; bz:=bz xor bsum; end;
    dst:=p*2;
...
{ (1+t+t^-1)*sum(F_0..F_(degree-1))=F_degree+F_(degree-1)+1. }
if summed then
  begin
  carry:=0;
  for p:=words-1 downto 0 do
    begin
    value:=a[p] xor b[p]; an:=0;
    if p+1<words then an:=a[p+1] xor b[p+1];
    value:=(value shr 1) xor (an shl 31) xor ((carry xor (carry shr 1)) shl 31) xor ((carry and 1) shl 30);
    value:=value xor (value shr 1);
    value:=value xor (value shr 3); value:=value xor (value shr 6);
    value:=value xor (value shr 12); value:=value xor (value shr 24);
    fa[p]:=value; carry:=value;
    end;
  r:=fa;
  end
else r:=a;
